...
int main(int argc, const char *argv[])
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        if (bFocusOnVehicle)
        {
            keypoints.erase(std::remove_if(keypoints.begin(), keypoints.end(), [&vehicleRect](const cv::KeyPoint &r){return !(vehicleRect.contains(r.pt));}), keypoints.end());
        }
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            string matcherType = "MAT_BF";        // MAT_BF, MAT_FLANN (Use MAT_FLANN for SIFT descriptor)
